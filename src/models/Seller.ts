...
export const SellerModel = {
    ...
    async getById(id: number) {
        const { data, error } = await supabase.from("Seller").select("*").eq("id", id).single();
        if (error) throw new Error(error.message);
        return data;
    },

    async create(seller: { name: string }) {
        const { data, error } = await supabase.from("Seller").insert([seller]).select();
        if (error) throw new Error(error.message);
        return data[0];
    },

    async update(id: number, seller: { name?: string }) {
        const { data, error } = await supabase.from("Seller").update(seller).eq("id", id).select();
        if (error) throw new Error(error.message);
        return data[0];
    },
