...
export const ProductModel = {
    ...
    async create(product: { product_name: string; product_price: number; product_imgurl?: string; seller_id?: number; advices_id?: number }) {
        const { data, error } = await supabase.from("Product").insert([product]).select();
        if (error) throw new Error(error.message);
        return data[0];
    },

    async update(product_id: number, product: Partial<{ product_name: string; product_price: number; product_imgurl: string; seller_id: number; advices_id: number }>) {
